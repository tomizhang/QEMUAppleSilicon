 ...
 *
 * Commands such as rAIC_EIR_MASK_SET/CLR assign each domain to a 32bit
 * register. When masking/unmasking-ing IRQ n, write to (aic_base +
 * command_reg_base + (n / 32) * 4) a uint32_t which has (n % 32)-th bit set,
 * command_reg_base is 0x4100 for rAIC_EIR_MASK_SET, 0x4180 for
 * rAIC_EIR_MASK_CLR.
 *
    ...
    WITH_QEMU_LOCK_GUARD(&s->mutex)
    {
        ...
        case rAIC_IPI_CLR: {
            ...
            for (i = 0; i < s->numCPU; i++) {
                if (val & (1 << i)) {
                    clear_bit(o->cpu_id,
                              (unsigned long *)&s->cpus[i].pendingIPI);
                }
            }
        ...
        case rAIC_IPI_DEFER_SET: {
            ...
            for (i = 0; i < s->numCPU; i++) {
                if (val & (1 << i)) {
                    set_bit(o->cpu_id,
                            (unsigned long *)&s->cpus[i].deferredIPI);
                }
            }
...
#ifdef AIC_DEBUG_NEW_IRQ
            if ((s->eir_mask[eir] | s->eir_mask_once[eir]) !=
                s->eir_mask[eir]) {
                for (int i = 0; i < 32; i++) {
        ...
        default:
            qemu_log_mask(LOG_UNIMP,
                          "AIC: Write to unspported reg 0x" HWADDR_FMT_plx
                          " cpu %u: 0x%x\n",
                          addr, o->cpu_id, val);
            break;
        }
